...
upload: build/$(PROJECT).bin
	...
	openocd -f ./stm32f4discovery.cfg -c "reset_config trst_only combined" -c "program build/$(PROJECT).elf verify reset exit" # For openocd 0.9

upload-pi-remote: build/$(PROJECT).elf
	./upload_remote_pi build/$(PROJECT).elf pi 10.42.0.199 22
